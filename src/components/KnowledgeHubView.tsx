...
interface KnowledgeHubViewProps {
  ...
  onTransferToRedTeam?: (item: KnowledgeItem) => void;
}
...
export const KnowledgeHubView: React.FC<KnowledgeHubViewProps> = ({
  ...
  const [editMacroCategory, setEditMacroCategory] = useState('통화정책/금리');
  const [editSentiment, setEditSentiment] = useState<'Bullish' | 'Bearish' | 'Neutral/Complex'>('Neutral/Complex');
  const [editScope, setEditScope] = useState<'Macro (거시)' | 'Micro (미시/산업)'>('Macro (거시)');
  ...
  const handleOpenEditModal = (item: KnowledgeItem) => {
    ...
    setEditSentiment(item.sentiment || 'Neutral/Complex');
    setEditScope(item.scope || 'Macro (거시)');
  };
  ...
  const handleSaveEdit = (e: React.FormEvent) => {
    ...
    const updated: KnowledgeItem = {
      ...
      content: editContent.trim() || editingItem.content,
      tags: editTags.split(',').map((t) => t.trim()).filter(Boolean),
      macroCategory: editMacroCategory || editingItem.macroCategory,
  ...
  return (
    <div id="knowledge-hub-view" className="space-y-6">
              ...
              <div className="rounded-2xl bg-slate-900/90 border border-slate-800 p-5 sm:p-6 space-y-5 shadow-xl">
                ...
                <div className="flex flex-wrap items-start justify-between gap-3 pb-4 border-b border-slate-800">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {onTransferToRedTeam && (
                      <button
                        onClick={() => onTransferToRedTeam(activeItem)}
                        className="px-3 py-1.5 rounded-lg bg-rose-500/20 hover:bg-rose-500/30 text-rose-300 border border-rose-500/40 text-xs font-semibold flex items-center gap-1.5 transition cursor-pointer shadow-sm"
                        title="이 가설과 분석을 레드팀 토론(Red Team Chat)으로 이관하여 재토론"
                      >
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 text-xs font-medium flex items-center gap-1 transition cursor-pointer"
                      title="노트 및 태그 수정"
                    >
                    ...
                    <button
                      onClick={() => setDeletingItemId(activeItem.id)}
                  ...
                  <div className="p-4 rounded-xl bg-slate-950 border border-amber-500/30 space-y-3">
                    ...
                    {activeItem.thesisReview.retrospectiveLessons && (
                      <div className="p-2.5 rounded-lg bg-amber-950/20 border border-amber-900/40 text-xs text-amber-200">
                        <strong className="text-amber-300 block mb-1">사후 회고 및 편향 피드백:</strong>
                        <MarkdownRenderer content={activeItem.thesisReview.retrospectiveLessons} />
                      </div>
                    )}
                  </div>
                )}
                ...
                {/* Content Body */}
                <div className="p-4 rounded-xl bg-slate-950 border border-slate-800/80 text-xs sm:text-sm text-slate-200 leading-relaxed max-h-64 overflow-y-auto">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-0.5">
                        노트에 누적된 지식을 기반으로 3대 탐색 조건(반대편 시각·도미노 파급·100년 역사)을 자동 발굴합니다.
                      </p>
                    </div>
                    ...
                    >
                      {isCurating ? (
                        ...
                        <>
                          ...
                          <span>연관 자료 AI 자동 탐색 실행</span>
                        </>
                      )}
                    </button>
                  </div>

                  {/* 3대 자동 탐색 조건 및 동작 원리 안내 배너 */}
                  <div className="p-3.5 rounded-xl bg-slate-950/80 border border-blue-900/40 space-y-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-[11px]">
                      <div className="p-2.5 rounded-lg bg-slate-900 border border-rose-900/30">
                        <span className="font-bold text-rose-300 flex items-center gap-1 mb-1">
                          <ShieldAlert className="w-3 h-3 text-rose-400" />
                          조건 1: 반대편 시각 (Anti-Consensus)
                        </span>
                        <p className="text-slate-400 leading-snug">
                          상승론(Bull)에 치우쳐 있다면 의도적으로 하방 리스크(Bear)와 음영 지대의 비판적 리포트를 찾아 균형을 맞춥니다.
                        </p>
                      </div>
                      <div className="p-2.5 rounded-lg bg-slate-900 border border-indigo-900/30">
                        <span className="font-bold text-indigo-300 flex items-center gap-1 mb-1">
                          <Layers className="w-3 h-3 text-indigo-400" />
                          조건 2: 시차적 파급 효과 (Macro Cross-Theme)
                        </span>
                        <p className="text-slate-400 leading-snug">
                          금리·환율·원자재 거시 지표나 인접 섹터로 번져나가는 2차·3차 도미노 효과 데이터를 추적합니다.
                        </p>
                      </div>
                      <div className="p-2.5 rounded-lg bg-slate-900 border border-amber-900/30">
                        <span className="font-bold text-amber-300 flex items-center gap-1 mb-1">
                          <History className="w-3 h-3 text-amber-400" />
                          조건 3: 100년 역사적 거울 (Historical Rhymes)
                        </span>
                        <p className="text-slate-400 leading-snug">
                          1970년대 오일쇼크, 1985 플라자합의, 2008 금융위기 등 메커니즘이 닮아 있는 역사적 대조 사건을 연결합니다.
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  >
                    <div className="flex flex-wrap items-center justify-between gap-2 pb-3 border-b border-slate-800">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {onTransferToRedTeam && (
                          <button
                            onClick={() => onTransferToRedTeam(item)}
                            className="px-2.5 py-1 rounded-lg bg-rose-500/20 hover:bg-rose-500/30 text-rose-300 text-xs font-semibold flex items-center gap-1 transition cursor-pointer border border-rose-500/40"
                            title="이 가설과 복습 일지를 레드팀 토론으로 즉시 이관하여 재토론"
                          >
                        ...
                        <button
                          onClick={() => onLoadIntoPrism(item)}
                          className="px-2.5 py-1 rounded-lg bg-blue-500/15 hover:bg-blue-500/25 text-blue-300 text-xs font-medium flex items-center gap-1 transition cursor-pointer border border-blue-500/30"
                          title="프리즘 재분석 실행"
                        >
                        ...
                        <button
                          ...
                          className="px-2.5 py-1 rounded-lg bg-amber-500/15 hover:bg-amber-500/25 text-amber-300 text-xs font-medium flex items-center gap-1 transition cursor-pointer border border-amber-500/30"
                        >
                        ...
                        <button
                          ...
                          className="px-2 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-medium flex items-center gap-1 transition cursor-pointer"
                          title="노트 수정"
                        >
                        ...
                        <button
                          ...
                          className="p-1 rounded-lg bg-slate-800 hover:bg-red-950/40 text-slate-400 hover:text-red-400 transition cursor-pointer"
                          title="항목 삭제"
                        >
      ...
      {/* Modal: Edit Knowledge Item / Note */}
      {editingItem && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-lg w-full p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between pb-2 border-b border-slate-800">
            ...
            <form onSubmit={handleSaveEdit} className="space-y-3 text-xs">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setEditMacroCategory(e.target.value)}
                    className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-slate-200 focus:outline-none"
                  >
                    <option value="통화정책/금리">통화정책/금리</option>
                    <option value="외환/환율">외환/환율</option>
              ...
              <div>
                <label className="text-slate-400 block mb-1">시장 관점 (Sentiment)</label>
                <select
                  value={editSentiment}
                  onChange={(e) => setEditSentiment(e.target.value as any)}
                  className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-slate-200 focus:outline-none"
                >
                  <option value="Bullish">Bullish (상승/낙관)</option>
                  <option value="Bearish">Bearish (하락/경계)</option>
                  <option value="Neutral/Complex">Neutral/Complex (중립/복합)</option>
                </select>
              </div>

              <div>
                <label className="text-slate-400 block mb-1">본문 내용 / 가설 요약</label>
                <textarea
                  value={editContent}
                  onChange={(e) => setEditContent(e.target.value)}
                  rows={5}
                  className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-slate-200 focus:outline-none focus:ring-1 focus:ring-blue-500 resize-none leading-relaxed"
                  required
                />
              </div>
      ...
      {deletingItemId && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-slate-900 border border-red-900/50 rounded-2xl max-w-sm w-full p-6 space-y-4 shadow-2xl">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-rose-500/20 text-rose-400 flex items-center justify-center border border-rose-500/30">
                <Trash2 className="w-5 h-5" />
              </div>
              <div>
                <h3 className="text-sm font-bold text-white">서고 항목 삭제</h3>
                <p className="text-xs text-slate-400">정말 이 지식 항목을 삭제하시겠습니까?</p>
              </div>
            </div>

            <p className="text-xs text-slate-300 bg-slate-950 p-3 rounded-xl border border-slate-800">
              삭제된 분석 노트 및 가설 복습 일지는 복원할 수 없으며, 서고 타임라인에서도 영구 제외됩니다.
            </p>
            ...
            <div className="flex items-center justify-end gap-2 pt-2">
              ...
              <button
                ...
                className="px-4 py-1.5 rounded-xl bg-rose-600 hover:bg-rose-500 text-white text-xs font-bold cursor-pointer shadow-lg shadow-rose-950/40"
              >
                삭제 확인
              </button>
            </div>
